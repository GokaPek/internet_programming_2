...
import com.example.demo.items.api.ItemGroupedDto;
...
public class UserProfileController {
    ...
    public String getProfile(
        ...
        List<ItemGroupedDto> tops = itemService.getTop(0, 5).stream().map(this::toGroupedDto).toList();
        model.addAttribute("tops", tops);

        final long userId = principal.getId();
        final Map<String, Object> attributes = PageAttributesMapper
                .toAttributes(applicationService.getAll(userId, 0, page, 5), this::toAppDto);
        ...
        // model.addAttribute("tops", itemService.getTop(0, 5).stream().map(this::toGroupedDto).toList());
        return PROFILE_VIEW;
    }

    @GetMapping("/profile")
    public String profile(Model model) {
        // List<LineGroupedDto> linesGr = lineService.getTop(0,
        // 5).stream().map(this::toGroupedDto).toList();
